...
public class URIEncoder {
    ...
    public static String encodeUri(String uri) {
        final StringBuffer result = new StringBuffer(2 * uri.length());
    ...
    public static void encodeUri(String uri, StringBuffer buf) {
        for (int i = 0; i < uri.length(); i++) {
            ...
            if ( (c!='%') && (c!='#') && ((ESCAPE_CHARS.indexOf(c) >= 0) || (cInt <= 0x20))) {
                // Escape character
                buf.append('%');

                final String hexVal = Integer.toHexString(cInt);
    ...
    public static String decodeUri(String uri) {
        final StringBuffer result = new StringBuffer(uri.length());
    ...
    public static void decodeUri(String uri, StringBuffer buf) {
        ...
        if (indexOfHash == -1) {
            ...
            final String baseUri = uri.substring(0, indexOfHash);
            final String fragId = uri.substring(indexOfHash);
    ...
    private static void _decodeUri(String uri, StringBuffer buf) {
        ...
        while (percentIdx != -1) {
            ...
            final String xx = uri.substring(percentIdx + 1, percentIdx + 3);
            final int c = Integer.parseInt(xx, 16);
            buf.append((char) c);
